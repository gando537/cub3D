...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/04/27 19:24:03 by mdiallo           #+#    #+#             */
/*   Updated: 2021/04/27 19:24:05 by mdiallo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_window	*init_game_window_pone(void)
{
	t_window	*new_win_infos;

	new_win_infos = malloc(sizeof(t_window));
	if (!new_win_infos)
		exit(1);
	ft_bzero(new_win_infos, sizeof(t_window));
	new_win_infos->mlx_ptr = mlx_init();
	if (!(new_win_infos->mlx_ptr))
		return (ERROR);
...
static int	init_game_window_ptwo(t_window *win_infos)
{
	char	*map_string;
	...
	win_infos->win_ptr = mlx_new_window(win_infos->mlx_ptr,
			win_infos->width, win_infos->height, "Cub3D");
	if (!(win_infos->win_ptr))
		return (ERROR);
	win_infos->img = new_image(win_infos, win_infos->width,
			win_infos->height);
	if (!(win_infos->img))
		return (ERROR);
	return (SUCCES);
...
int	main(int argc, char **argv)
{
	...
	if (argc < 2)
		putstr_info_cmd();
	win_infos = init_game_window_pone();
	if (!(win_infos)
